...
ps_data = {'stock': 0, 'price': 0.0}
try:
    with open('/tmp/ps_info_tu100.tsv', 'r') as f:
        lines = f.readlines()
        if len(lines) > 1:
            ...
            if len(parts) >= 6:
                ...
                ps_data['stock'] = stock
                ps_data['price'] = final_price
except Exception as e:
    print("Error reading PS data:", e)
...
ws.append(["SKU", "Giacenza (Real-Time PS)", "Venduti (da Ott 2025)", "Prezzo Scontato Attuale (€)"])
ws.append([sku, ps_data['stock'], sold, round(ps_data['price'], 2)])

file_path = f"/root/.openclaw/workspace-marketing/Report_INFO_{sku}.xlsx"
wb.save(file_path)
