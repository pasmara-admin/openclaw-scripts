...
def main():
    ...
    # Prendi le vendite raggruppate per "parent_reference". 
    # Usiamo solo r.type_id = 'V' per calcolare esclusivamente il fatturato derivato dai prodotti (escludendo i costi di spedizione 'C' o 'S' associati alla riga o all'ordine).
    query_sales = """
        SELECT 
        ...
        WHERE o.date >= CURDATE() - INTERVAL 30 DAY
          ...
          AND r.type_id = 'V'  -- SOLO PRODOTTI, escluse spedizioni e servizi
          AND p.id IN (
    ...
    for row in sales_data:
        ...
        stats[parent_sku] = {
            ...
            'Fatturato Netto Prodotti Drop (Ultimi 30gg)': float(row['revenue']),
    ...
    for parent_sku, data in stats.items():
        revenue = data['Fatturato Netto Prodotti Drop (Ultimi 30gg)']
    ...
    top_performers = sorted(top_performers, key=lambda x: x['Fatturato Netto Prodotti Drop (Ultimi 30gg)'], reverse=True)
    
    df = pd.DataFrame(top_performers)
    path = "/root/.openclaw/workspace-marketing/Drop_TopPerformers_LowIncidence_Halo.xlsx"
    df.to_excel(path, index=False)
    
    print(f"File generato (NO SPEDIZIONI). {len(top_performers)} parent products trovati.")
