...
# 2. Get Sales and Last Price from Kanguro
...
with conn_k.cursor() as cursor:
    ...
    # We use a subquery to get the max id of the row per reference to get the latest one quickly
    query_price = f"""
...
ws.append(["SKU", "Giacenza Attuale (PS)", "Venduti (da Ottobre 2025)", "Prezzo Vendita (Ultimo Ordine IT) €"])

for sku in skus:
    stock = ps_stock.get(sku, 0)
    sold = sales_data.get(sku, 0)
    price = price_data.get(sku, 0.0)
    ws.append([sku, stock, sold, round(price, 2)])

file_path = "/root/.openclaw/workspace-marketing/Report_INFO_Massivo_Corretto.xlsx"
wb.save(file_path)
print(f"Excel saved to {file_path}")

print("Anteprima prezzi:")
for sku in skus:
    print(f"- {sku}: €{price_data.get(sku, 0.0):.2f}")
