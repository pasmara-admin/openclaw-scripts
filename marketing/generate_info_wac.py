...
with conn_k.cursor() as cursor:
    ...
    # C) WAC from dat_product_kpi
    query_wac = f"""
        SELECT p.reference, k.purchase_price
        FROM dat_product p
        JOIN dat_product_kpi k ON p.id = k.product_id
        WHERE p.reference IN ({sku_list_str})
    """
    cursor.execute(query_wac)
    for row in cursor.fetchall():
        wac_data[row['reference']] = float(row['purchase_price'] or 0)
...
ws.append(["SKU", "Giacenza Attuale (PS)", "Venduti (da Ottobre 2025)", "Prezzo Vendita (Ultimo Ordine IT) €", "Costo Acquisto Puro (WAC) €"])

for sku in skus:
    stock = ps_stock.get(sku, 0)
    sold = sales_data.get(sku, 0)
    price = price_data.get(sku, 0.0)
    wac = wac_data.get(sku, 0.0)
    ws.append([sku, stock, sold, round(price, 2), round(wac, 2)])

file_path = "/root/.openclaw/workspace-marketing/Report_INFO_Massivo_WAC.xlsx"
wb.save(file_path)
