...
def get_classified_data():
    ...
    SELECT 
        ...
        CASE 
            WHEN ls.tracking_id IS NULL THEN 'NO AFFIDO INFORMATICO'
            WHEN dr.id IS NULL OR (dr.delivered = 0 AND dr.actual_delivery_date IS NULL AND dr.is_manual = 0) THEN 'NO AFFIDO REALE'
            ELSE 'CON ESITO'
    ...
    LEFT JOIN lgs_delivery_result dr ON ls.tracking_id = dr.tracking_id
    ...
    # Filter only those without esito
    df_filtered = df[df['classificazione'].isin(['NO AFFIDO INFORMATICO', 'NO AFFIDO REALE'])]
    
    return df_filtered

if __name__ == "__main__":
    try:
        df = get_classified_data()
        if not df.empty:
            output_path = "/root/.openclaw/workspace-marketing/ritorni_senza_esito_2025.xlsx"
            df.to_excel(output_path, index=False)
            print(f"SUCCESS: Report generated at {output_path}")
            print(f"Rows found: {len(df)}")
            print(df['classificazione'].value_counts())
        else:
