...
for index, row in df.iterrows():
    ...
    # Process only Fatture e Ricevute
    primary_docs = [d for d in docs if d['type_id'] in (1, 3)]
    
    # Sort docs to prefer valid string numbers over "NULL"
    # Or actually keep all of them, let's keep all primary docs for this reference
    ...
    # Calculate total sum of documents for this specific order
    total_docs_sum = sum(d['total'] for d in primary_docs if d.get('total') is not None)
    
    # If the order is 725030147, it might have multiple docs. Let's check logic:
    # Actually the diff_importi shouldn't be calculated per-document subtracting the FULL excel import from ONE document if there are multiple!
    # M wants: se ci sono 2 fatture, la "Differenza" deve essere = Excel - (Somma Fatture) e stampata magari su ogni riga, 
    # o calcolata proporzionalmente? "La differenza deve tener conto che le fatture sono due... la differenza sarebbe Euro 1.4"
    # That means: Differenza = Importo Excel - Somma di tutte le fatture.
    
    diff_importi = importo_excel - total_docs_sum if primary_docs[0].get('total') is not None else None
    ...
    for doc in primary_docs:
        ...
        result_rows.append({
            ...
            'Importo (Kanguro Singolo)': doc_total,
            'Importo (Kanguro Totale Ordine)': total_docs_sum if doc_total is not None else None,
            'Importo (Excel Col G)': importo_excel,
            'Differenza Importi (Excel - Kanguro Totale)': diff_importi,
...
out_path = '/root/.openclaw/workspace-finance/Associazione_PayPlug_Kanguro_V4.csv'
