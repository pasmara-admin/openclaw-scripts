    ...
    if line:
        ...
        if len(parts) == 5:
            ...
            if ref not in ref_to_docs:
                ref_to_docs[ref] = []
            ref_to_docs[ref].append({'doc': f"{full_number} ({type_name})", 'date': date, 'total': float(total), 'type_id': type_id})
...
# Drop empty rows and format dates to strings for CSV so time isn't weird if 00:00:00
...
print(f"File generated. Average delay: {res_df['Ritardo Emissione Doc (Giorni)'].mean():.2f}")
