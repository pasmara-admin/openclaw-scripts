...
# Rendo negativi i totali delle Note di Credito e Rimborsi
...
try:
    ...
    df_pp['Numero Ordine'] = pd.to_numeric(df_pp['metadata_Order'], errors='coerce').fillna(0).astype(int).astype(str)
    
    df_pp_agg = df_pp.groupby('Numero Ordine').agg({
        'Importo (€)': 'sum',
...
out_file = '/root/.openclaw/workspace-finance/reconciliation/Master_Reconciliation_Fatture_Sito_Ott_Nov_2025_v3.xlsx'
