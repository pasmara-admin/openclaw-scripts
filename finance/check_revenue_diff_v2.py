...
print("--- Totali da Report_Revenue ---")
print(df_rev_agg.sort_values('Revenue Gross', ascending=False).to_string())

print("\n--- Totali da Master Reconciliation (Fatture) ---")
f = '/root/.openclaw/workspace-finance/reconciliation/Master_Reconciliation_Fatture_Sito_Ott_Nov_2025_v4.xlsx'
df_rec = pd.read_excel(f)
df_rec_agg = df_rec.groupby('Metodo Pagamento v2')['Fatturato Lordo (Kanguro)'].sum().reset_index()
