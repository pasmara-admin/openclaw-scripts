...
def get_data(lookback_days):
    ...
    # Note: Using >= to capture products that had sales within the lookback window
    ...
    for _, row in df_sales_nd.iterrows():
        ...
        # Determine the soldout date (the first day it hit 0 or the most recent transition)
        query_soldout = f"""
        ...
        # Look at the 30 days PRIOR to the soldout date to get the baseline velocity
...
if __name__ == "__main__":
    ...
    for days in [1, 7, 14, 30]:
        ...
        if df is not None:
            ...
            period_label = "Ieri" if days == 1 else f"Ultimi {days} giorni"
            summary.append({
                'Periodo': period_label,
                'Quantità Medie Giornaliere': round(avg_qty, 2),
                'GMV Medio Giornaliero': round(avg_gmv, 2)
            })
    ...
    if all_data:
        ...
        # Use simple table format for console/chat output
